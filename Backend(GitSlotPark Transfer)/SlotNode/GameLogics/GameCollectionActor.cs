...
namespace SlotGamesNode.GameLogics
{
    public class GameCollectionActor : ReceiveActor
    {
        ...
        protected void createGameLogicActors()
        {
            ...
            foreach (KeyValuePair<GAMEID, IActorRef> dicGameLogicActor in _dicGameLogicActors)
            {
                dicGameLogicActor.Value.Tell(new DBProxyInform(_dbReader, _dbWriter, _redisWriter));
                dicGameLogicActor.Value.Tell("loadSetting");

                _hashAllChildActors.Add(dicGameLogicActor.Value.Path.Name);
                Context.Watch(dicGameLogicActor.Value);
            }
        }
        ...
        private async Task onLoadSpinDatabase(LoadSpinDataRequest request)
        {
            try
            {
                var infoDocuments = await Context.ActorSelection("/user/spinDBReaders").Ask<List<BsonDocument>>(new ReadInfoCollectionRequest(), TimeSpan.FromSeconds(10.0));
