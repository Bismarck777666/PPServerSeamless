...
{
    public class FiveLionsMegaGameLogic : BaseSelFreePPSlotGame
    {
        #region 게임고유속성값
        ...
        public FiveLionsMegaGameLogic()
        {
            _gameID     = GAMEID.FiveLionsMega;
            GameName    = "FiveLionsMega";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in FiveLionsMegaGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }

                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in FiveLionsMegaGameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in FiveLionsMegaGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
