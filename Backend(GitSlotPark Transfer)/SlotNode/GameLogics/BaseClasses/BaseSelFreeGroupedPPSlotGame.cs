    ...
    public class BaseSelFreeGroupedPPSlotGame : BaseSelFreePPSlotGame
    {
        ...
        protected override async Task onLoadSpinData(BsonDocument infoDocument)
        {
            try
            {
                ...
                List<BsonDocument> seqDocuments = await Context.System.ActorSelection("/user/spinDBReaders").Ask<List<BsonDocument>>(new SelectAllSequenceRequest(GameName), TimeSpan.FromSeconds(10.0));
        ...
        protected override async Task<OddAndIDData> selectRandomOddAndID(int websiteID,BasePPSlotBetInfo betInfo,bool isMoreBet)
        {
            ...
            else if (_groupNaturalCounts.ContainsKey(groupID) && _groupNaturalCounts[groupID] > 0)
        ...
        protected override async Task<BasePPSlotSpinData> selectRangeSpinData(int websiteID,double minOdd,double maxOdd,BasePPSlotBetInfo betInfo)
        {
            ...
            BsonDocument document = await Context.System.ActorSelection("/user/spinDBReaders").Ask<BsonDocument>(new SelectSelFreeGroupSpinRangeRequest(GameName, minOdd, maxOdd, getRangeID(minOdd, maxOdd), groupID), TimeSpan.FromSeconds(10.0));
        ...
        protected override async Task<BasePPSlotSpinData> selectEmptySpin(int websiteID,BasePPSlotBetInfo betInfo)
        {
            ...
            BsonDocument document = await Context.System.ActorSelection("/user/spinDBReaders").Ask<BsonDocument>(new SelectSpinDataByIDRequest(GameName, id), TimeSpan.FromSeconds(10.0));
            BasePPSlotSpinData spinData = convertBsonToSpinData(document);
            if (spinData.SpinOdd > 0.0)
            {
                SelectedGroupSequence sequenceGroup = getCurrentSequenceGroup(betInfo);
                sequenceGroup.CreditMoney -= spinData.SpinOdd * (double)betInfo.TotalBet;
            }

            processSlotSpinData(spinData, grpSample.Index, grpSample.Group, grpSample.CumulatedGroupSum);
            BasePPSlotSpinData basePpSlotSpinData = spinData;
