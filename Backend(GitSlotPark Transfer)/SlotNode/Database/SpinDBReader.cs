...
namespace SlotGamesNode.Database
{
    public class SpinDBReader : ReceiveActor
    {
        ...
        private async Task onSelectSpinDataByID(SelectSpinDataByIDRequest request)
        {
            try
            {
                var dbClient    = new MongoClient(_dbConnString);
                var spinDB      = dbClient.GetDatabase(_dbName, null);
                ...
                BsonDocument spinData = await collection.Find(filter).FirstAsync();
        ...
        private async Task onSelectSpinTypeOddRange(SelectSpinTypeOddRangeRequest request)
        {
            try
            {
                ...
                {
                    FilterDefinition<BsonDocument> filter = Builders<BsonDocument>.Filter.Exists((FieldDefinition<BsonDocument>)"puri", false);
        ...
        private async Task onSelectSelFreeSpinRange(SelectSelFreeSpinRangeRequest request)
        {
            try
            {
                ...
                if (request is SelectSelFreeGroupSpinRangeRequest)
                {
                    ...
                    document = await collection.AsQueryable().Where(x => x["groupid"] == groupReq.GroupID && x["spintype"] == 0 && x["odd"] >= request.MinOdd && x["odd"] <= request.MaxOdd || x["groupid"] == groupReq.GroupID && x["spintype"] == 100 && (x["ranges"] as BsonArray).Contains(request.RangeID)).Sample(1).FirstOrDefaultAsync();
                }
                else if (request is SelectSelFreeOnlyPuri0SpinRangeRequest)
                {
                    FilterDefinition<BsonDocument> filter = Builders<BsonDocument>.Filter.Exists("puri", false);
                    document = await collection.AsQueryable().Where(x => filter.Inject() && x["spintype"] == 0 && x["odd"] >= request.MinOdd && x["odd"] <= request.MaxOdd || (filter.Inject() || x["puri"] == 0) && x["spintype"] == 100 && (x["ranges"] as BsonArray).Contains(request.RangeID)).Sample(1).FirstOrDefaultAsync();
                }
                else
                    document = await collection.AsQueryable().Where(x => x["spintype"] == 0 && x["odd"] >= request.MinOdd && x["odd"] <= request.MaxOdd || x["spintype"] == 100 && (x["ranges"] as BsonArray).Contains(request.RangeID)).Sample(1).FirstOrDefaultAsync();
        ...
        private async Task onSelectAllSequence(SelectAllSequenceRequest request)
        {
            try
            {
                ...
                List<BsonDocument> documents = await collection.Find(_ => true).ToListAsync<BsonDocument>();
                Sender.Tell(documents);
            }
            catch (Exception ex)
            {
