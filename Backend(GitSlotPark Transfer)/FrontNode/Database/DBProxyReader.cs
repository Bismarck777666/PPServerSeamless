...
namespace FrontNode.Database
{
    public class DBProxyReader : ReceiveActor
    {
        ...
        {
            string[] strParts = strContent.Split(new string[1] { "&" }, StringSplitOptions.RemoveEmptyEntries);
            
            Dictionary<string, string> dicReturn = new Dictionary<string, string>();
            for (int i = 0; i < strParts.Length; i++)
            {
                string[] strParamValues = strParts[i].Split(new string[1] { "=" }, StringSplitOptions.RemoveEmptyEntries);
