...
namespace QueenApiNode.Database
{
    public class DBProxyWriteWorker : ReceiveActor
    {
        ...
        private async Task<int> updateAgentBalances(SqlConnection connection)
        {
            ...
            try
            {
                updateItems = await Context.Parent.Ask<List<AgentScoreUpdateItem>>("PopAgentScoreUpdates", TimeSpan.FromSeconds(5.0));
                if (updateItems == null)
                    return 0;
        ...
        {
            List<AgentMoneyChangeItem> insertItems = await Context.Parent.Ask<List<AgentMoneyChangeItem>>("PopAgentMoneyChangeItems", TimeSpan.FromSeconds(5.0));
            if (insertItems == null)
                return 0;
        ...
        {
            List<UserMoneyChangeItem> insertItems = await Context.Parent.Ask<List<UserMoneyChangeItem>>("PopUserMoneyChangeItems", TimeSpan.FromSeconds(5.0));
            if (insertItems == null)
                return 0;
