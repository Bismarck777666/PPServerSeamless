...
namespace QueenApiNode.Database
{
    internal class DBProxyWriter : ReceiveActor
    {
        ...
        public DBProxyWriter(string strConnStriing)
        {
            ...
            Receive<AgentScoreUpdateItem>       (_ => WriterSnapshot.Instance.updateAgentScore(_));
            Receive<List<AgentScoreUpdateItem>> (_ => WriterSnapshot.Instance.pushAgentScoreUpdates(_));
            Receive<AgentMoneyChangeItem>       (_ => WriterSnapshot.Instance.insertAgentMoneyChangeItem(_));
            Receive<List<AgentMoneyChangeItem>> (_ => WriterSnapshot.Instance.pushAgentMoneyChangeItems(_));
            Receive<UserMoneyChangeItem>        (_ => WriterSnapshot.Instance.insertUserMoneyChangeItem(_));
            Receive<List<UserMoneyChangeItem>>  (_ => WriterSnapshot.Instance.pushUserMoneyChangeItems(_));
        ...
        private void processCommand(string strCommand)
        {
            ...
            {
                Sender.Tell(WriterSnapshot.Instance.popAgentScoreUpdates());
            }
            else if(strCommand == "PopAgentMoneyChangeItems")
            {
                Sender.Tell(WriterSnapshot.Instance.popAgentMoneyChangeItems());
            }
            else if(strCommand == "PopUserMoneyChangeItems")
            {
                Sender.Tell(WriterSnapshot.Instance.popUserMoneyChangeItems());
            }
