...
{
    public class AllwaysHotDiceBetInfo : BaseAmaticSlotBetInfo
    {
        public override int RelativeTotalBet    => 1;
    }

    class AllwaysHotDiceGameLogic : BaseAmaticSlotGame
    {
        #region 게임고유속성값
        ...
        public AllwaysHotDiceGameLogic()
        {
            _gameID     = GAMEID.AllwaysHotDice;
            GameName    = "AllwaysHotDice";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strUserID, Currencies currency)
        {
            try
            {
                AllwaysHotDiceBetInfo betInfo   = new AllwaysHotDiceBetInfo();
                ...
                {
                    _logger.Error("{0} betInfo 0 or infinite in AllwaysHotDiceGameLogic::readBetInfoFromMessage", strUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in AllwaysHotDiceGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
