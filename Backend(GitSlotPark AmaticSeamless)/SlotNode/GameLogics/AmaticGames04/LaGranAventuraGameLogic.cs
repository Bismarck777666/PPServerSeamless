    ...
    class LaGranAventuraGameLogic : BaseSelFreeAmaticSlotGame
    {
        ...
        protected override void onFreeSpinOptionSelectRequest(string strUserID, int websiteID, GITMessage message, double userBalance)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} option select error in LaGranAventuraGameLogic::onFreeSpinOptionSelectRequest", strGlobalUserID);
                }
                else
                {
        ...
        protected override string buildInitString(string strGlobalUserID, double balance, Currencies currency)
        {
            ...
            LaGranAventuraInitPacket initPacket = new LaGranAventuraInitPacket(initString, Cols, FreeCols, ReelSetColBitNum, ReelSetBitNum);
        ...
        public class LaGranAventuraInitPacket : InitPacket
        {
            ...
            public LaGranAventuraInitPacket(string message, int reelCnt, int freeReelCnt, int reelsetColBitCnt, int reelsetBitCnt) : base(message, reelCnt, freeReelCnt, reelsetColBitCnt, reelsetBitCnt)
            {
                extrastr    = "00150000015000001500000";
                curpoint    = curpoint + extrastr.Length;
            }
