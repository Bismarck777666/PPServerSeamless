...
namespace SlotGamesNode.GameLogics
{
    class BigPandaGameLogic : BaseAmaticMultiBaseExtra1Game
    {
        ...
        protected override async Task onProcMessage(string strUserID, int companyID, GITMessage message, double userBalance, Currencies currency)
        {
            if (message.MsgCode == (ushort)CSMSG_CODE.CS_AMATIC_FSOPTION)
                await onDoSpin(strUserID, companyID, message, userBalance,currency);
            else
                await base.onProcMessage(strUserID, companyID, message, userBalance, currency);
        }
    }
}
