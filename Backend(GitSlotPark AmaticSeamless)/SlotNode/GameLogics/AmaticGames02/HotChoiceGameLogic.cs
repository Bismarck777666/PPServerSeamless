...
{
    class HotChoiceGameLogic : BaseSelFreeAmaticSlotGame
    {
        #region 게임고유속성값
        ...
        public HotChoiceGameLogic()
        {
            _gameID     = GAMEID.HotChoice;
            GameName    = "HotChoice";
        }

        protected override string addStartWinToResponse(string strResponse, BasePPSlotStartSpinData startSpinData)
        {
            HotChoicePacket firstPacket = new HotChoicePacket(startSpinData.SpinStrings[0], Cols, FreeCols);
            HotChoicePacket packet = new HotChoicePacket(strResponse, Cols, FreeCols);
        ...
        protected override void onFreeSpinOptionSelectRequest(string strUserID, int websiteID, GITMessage message, double userBalance)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} option select error in HotChoiceGameLogic::onFreeSpinOptionSelectRequest", strGlobalUserID);
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        HotChoicePacket packet = new HotChoicePacket(strSpinResponse, Cols, FreeCols);
            ...
            {
                _logger.Error("Exception has been occurred in HotChoiceGameLogic::onFreeSpinOptionSelectRequest {0}", ex);
            }
        }
        protected override string buildInitString(string strGlobalUserID, double balance, Currencies currency)
        {
            ...
            HotChoiceInitPacket initPacket = new HotChoiceInitPacket(initString, Cols, FreeCols, ReelSetColBitNum, ReelSetBitNum);
            ...
            if (_dicUserBetInfos.ContainsKey(strGlobalUserID) && _dicUserResultInfos.ContainsKey(strGlobalUserID))
            {
                ...
                if (betInfo.HasRemainResponse)
                {
                    HotChoicePacket amaPacket = new HotChoicePacket(spinResult.ResultString, Cols, FreeCols);
        ...
        {
            HotChoicePacket packet = null;
            double pointUnit = getPointUnit(betInfo);

            if (!string.IsNullOrEmpty(spinString))
            {
                packet = new HotChoicePacket(spinString, Cols, FreeCols);
            ...
            {
                packet = new HotChoicePacket(Cols, FreeCols, (int)type, (int)LINES.Last());
                ...
                if (_dicUserBetInfos.ContainsKey(strGlobalUserID) && _dicUserResultInfos.ContainsKey(strGlobalUserID))
                {
                    ...
                    HotChoicePacket oldPacket = new HotChoicePacket(spinResult.ResultString, Cols, FreeCols);
        ...
        protected override string buildSpinString(AmaticPacket packet)
        {
            ...
            HotChoicePacket hotChoicePacket = null;
            if (packet is HotChoicePacket)
                hotChoicePacket = packet as HotChoicePacket;
            else
                hotChoicePacket = new HotChoicePacket(Cols, FreeCols, (int)packet.messagetype, (int)LINES.Last());
        ...
        public class HotChoicePacket : AmaticPacket
        {
            public long     freeoptionsymbol     { get; set; }
            public HotChoicePacket(string message, int reelCnt, int freeReelCnt) : base(message, reelCnt, freeReelCnt)
            {
            ...
            public HotChoicePacket(int reelCnt, int freeReelCnt, int msgType, int lineCnt) : base(reelCnt, freeReelCnt, msgType, lineCnt)
            {
                freeoptionsymbol    = 0;
                curpoint            = curpoint + 2;
            }
        ...
        public class HotChoiceInitPacket : InitPacket
        {
            public long     freeoptionsymbol    { get; set; }

            public HotChoiceInitPacket(string message, int reelCnt, int freeReelCnt, int reelsetColBitCnt, int reelsetBitCnt) : base(message, reelCnt, freeReelCnt, reelsetColBitCnt, reelsetBitCnt)
            {
                freeoptionsymbol    = 0;
                curpoint            = curpoint + 2;
            }
