...
{
    public class SakuraSecretBetInfo : BaseAmaticSlotBetInfo
    {
        public override int RelativeTotalBet    => 1;
    }

    class SakuraSecretGameLogic : BaseAmaticExtra1Game
    {
        #region 게임고유속성값
        ...
        public SakuraSecretGameLogic()
        {
            _gameID     = GAMEID.SakuraSecret;
            GameName    = "SakuraSecret";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strUserID, Currencies currency)
        {
            try
            {
                SakuraSecretBetInfo betInfo   = new SakuraSecretBetInfo();
                ...
                {
                    _logger.Error("{0} betInfo 0 or infinite in SakuraSecretGameLogic::readBetInfoFromMessage", strUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in SakuraSecretGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
