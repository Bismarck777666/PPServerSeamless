...
{
    class GoldenQuestGameLogic : BaseSelFreeAmaticSlotGame
    {
        #region 게임고유속성값
        ...
        public GoldenQuestGameLogic()
        {
            _gameID     = GAMEID.GoldenQuest;
            GameName    = "GoldenQuest";
        }

        protected override string addStartWinToResponse(string strResponse, BasePPSlotStartSpinData startSpinData)
        {
            GoldenQuestPacket firstPacket = new GoldenQuestPacket(startSpinData.SpinStrings[0], Cols, FreeCols);
            GoldenQuestPacket packet = new GoldenQuestPacket(strResponse, Cols, FreeCols);
        ...
        protected override void onFreeSpinOptionSelectRequest(string strUserID, int websiteID, GITMessage message, double userBalance)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} option select error in GoldenQuestGameLogic::onFreeSpinOptionSelectRequest", strGlobalUserID);
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        GoldenQuestPacket packet = new GoldenQuestPacket(strSpinResponse, Cols, FreeCols);
            ...
            {
                _logger.Error("Exception has been occurred in GoldenQuestGameLogic::onFreeSpinOptionSelectRequest {0}", ex);
            }
        }
        protected override string buildInitString(string strGlobalUserID, double balance, Currencies currency)
        {
            ...
            GoldenQuestInitPacket initPacket = new GoldenQuestInitPacket(initString, Cols, FreeCols, ReelSetColBitNum, ReelSetBitNum);
            ...
            if (_dicUserBetInfos.ContainsKey(strGlobalUserID) && _dicUserResultInfos.ContainsKey(strGlobalUserID))
            {
                ...
                if (betInfo.HasRemainResponse)
                {
                    GoldenQuestPacket amaPacket = new GoldenQuestPacket(spinResult.ResultString, Cols, FreeCols);
        ...
        {
            GoldenQuestPacket packet = null;
            double pointUnit = getPointUnit(betInfo);

            if (!string.IsNullOrEmpty(spinString))
            {
                packet = new GoldenQuestPacket(spinString, Cols, FreeCols);
            ...
            {
                packet = new GoldenQuestPacket(Cols, FreeCols, (int)type, (int)LINES.Last());
                ...
                if (_dicUserBetInfos.ContainsKey(strGlobalUserID) && _dicUserResultInfos.ContainsKey(strGlobalUserID))
                {
                    ...
                    GoldenQuestPacket oldPacket = new GoldenQuestPacket(spinResult.ResultString, Cols, FreeCols);
        ...
        protected override string buildSpinString(AmaticPacket packet)
        {
            ...
            GoldenQuestPacket goldenQuestPacket = null;
            if (packet is GoldenQuestPacket)
                goldenQuestPacket = packet as GoldenQuestPacket;
            else
                goldenQuestPacket = new GoldenQuestPacket(Cols, FreeCols, (int)packet.messagetype, (int)LINES.Last());

            AmaticEncrypt encrypt = new AmaticEncrypt();
            newSpinString = encrypt.WriteDec2Hex(newSpinString, goldenQuestPacket.freeoptionsymbol);
            return newSpinString;
        }

        public class GoldenQuestPacket : AmaticPacket
        {
            public long     freeoptionsymbol     { get; set; }
            public GoldenQuestPacket(string message, int reelCnt, int freeReelCnt) : base(message, reelCnt, freeReelCnt)
            {
            ...
            public GoldenQuestPacket(int reelCnt, int freeReelCnt, int msgType, int lineCnt) : base(reelCnt, freeReelCnt, msgType, lineCnt)
            {
                freeoptionsymbol    = 0;
                curpoint            = curpoint + 2;
            }
        ...
        public class GoldenQuestInitPacket : InitPacket
        {
            public long     freeoptionsymbol    { get; set; }

            public GoldenQuestInitPacket(string message, int reelCnt, int freeReelCnt, int reelsetColBitCnt, int reelsetBitCnt) : base(message, reelCnt, freeReelCnt, reelsetColBitCnt, reelsetBitCnt)
            {
                freeoptionsymbol    = 0;
                curpoint            = curpoint + 2;
            }
