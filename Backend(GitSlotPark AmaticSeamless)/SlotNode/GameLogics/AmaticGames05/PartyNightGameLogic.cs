...
{
    public class PartyNightBetInfo : BaseAmaticSlotBetInfo
    {
        public override int RelativeTotalBet    => 1;
    }

    class PartyNightGameLogic : BaseAmaticSlotGame
    {
        #region 게임고유속성값
        ...
        public PartyNightGameLogic()
        {
            _gameID     = GAMEID.PartyNight;
            GameName    = "PartyNight";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strUserID, Currencies currency)
        {
            try
            {
                PartyNightBetInfo betInfo   = new PartyNightBetInfo();
                ...
                {
                    _logger.Error("{0} betInfo 0 or infinite in PartyNightGameLogic::readBetInfoFromMessage", strUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in PartyNightGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
