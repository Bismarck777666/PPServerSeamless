...
namespace UserNode
{
    ...
    public class UserActor : ReceiveActor
    {
        ...
        private async Task onProcMessage(FromConnRevMessage fromConnRevMsg)
        {
            ...
            else if (message.MsgCode >= (ushort)CSMSG_CODE.CS_AMATIC_SLOTGAMESTART && message.MsgCode <= (ushort)CSMSG_CODE.CS_AMATIC_SLOTGAMEEND)
            {
                userConn.LastActiveTime = DateTime.Now;
        ...
        private async Task<WithdrawResponse> callWithdraw(int gameID, double amount, string roundID, string transactionID)
        {
            ...
            try
            {
                ...
                if(response.code == 0)
                    ...
                    _logger.Error("Withdraw callback returns no success code in UserActor::callWithdraw {0} {1} {2}", _agentID, response.code, response.message);
                return response;
            }
            catch (Exception ex)
            {
        ...
        private async Task<DepositResponse> callDeposit(int gameID, double amount, string roundID, string betTransactionID, string transactionID, bool roundEnd)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    _logger.Error("Withdraw callback returns no success code in UserActor::callDeposit {0} {1} {2}", _agentID, response.code, response.message);
                }
                return response;
            }
            catch (Exception ex)
        ...
        private async Task<WithdrawResponse> callBetWin(int gameID, double betAmount, double winAmount, string roundID, string transactionID)
        {
            ...
            try
            {
                ...
                if (response.code == 0)
                    ...
                    _logger.Error("Withdraw callback returns no success code in UserActor::callBetWin {0} {1} {2}", _agentID, response.code, response.message);
                return response;
            }
            catch (Exception ex)
            {
        ...
        private async Task procToUserMessage(ToUserMessage message, GITMessage gameMessage, UserConnection userConn)
        {
            ...
            if (message is ToUserResultMessage)
            {
                ...
                if((gameMessage.MsgCode == (ushort) CSMSG_CODE.CS_AMATIC_DOSPIN || gameMessage.MsgCode == (ushort)CSMSG_CODE.CS_AMATIC_DOGAMBLEHALF || gameMessage.MsgCode == (ushort)CSMSG_CODE.CS_AMATIC_DOGAMBLEPICK))
                {
                    if(!isSuccess)
                    {
                        GITMessage unauthorizationMsg = new GITMessage((ushort)SCMSG_CODE.SC_AMATIC_DOSPIN);
                        unauthorizationMsg.Append("-1unauthorization");
                        Sender.Tell(new SendMessageToUser(unauthorizationMsg, _balance, 0.0));

                        GITMessage errorResultMsg = new GITMessage((ushort)CSMSG_CODE.CS_AMATIC_NOTPROCDRESULT);
        ...
        public class UserConnection
        {
            ...
            public int          GameProvider    { get; set; } //进入的游戏公司
