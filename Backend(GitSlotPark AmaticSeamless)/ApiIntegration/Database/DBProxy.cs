...
namespace ApiIntegration.Database
{
    public class DBProxy : ReceiveActor
    {
        ...
        public DBProxy(Config config)
        {
            ...
            connStringBuilder.InitialCatalog    = config.GetString("dbname",    "amatic");
            connStringBuilder.UserID            = config.GetString("user",      "sa");
            connStringBuilder.Password          = config.GetString("pass",      "bismarck");
