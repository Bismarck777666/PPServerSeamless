    ...
    public class DBWriteWorker : ReceiveActor
    {
        ...
        private async Task<int> updateClaimedBonusItems(SqlConnection connection)
        {
            ...
            try
            {
                ...
                {
                    if (updateItem is ClaimedUserRangeEventItem)
                    {
                    ...
                    else if(updateItem is ClaimedUserFreeSpinEventItem)
                    {
                        SqlCommand command = new SqlCommand("UPDATE userfreespinevents SET processed=1, processeddate=@processeddate WHERE id=@id", connection, transaction);
                        command.Parameters.AddWithValue("@id",              (updateItem as ClaimedUserFreeSpinEventItem).EventID);
                        command.Parameters.AddWithValue("@processeddate",   (updateItem as ClaimedUserFreeSpinEventItem).ClaimedTime);
                        await command.ExecuteNonQueryAsync();
