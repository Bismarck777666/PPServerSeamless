        ...
        private async Task initializeMonitor()
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(_strConnString))
                {
                    ...
                    if (DBMonitorSnapshot.Instance.LastFreeSpinEventID == -1)
                    {
                        string strQuery = "SELECT TOP 1 id FROM userfreespinevents ORDER BY id DESC";
                        SqlCommand command = new SqlCommand(strQuery, connection);
                        using (DbDataReader reader = await command.ExecuteReaderAsync())
                        {
                            if (await reader.ReadAsync())
                            {
                                DBMonitorSnapshot.Instance.LastFreeSpinEventID = reader.GetInt64(0);
                            }
                        }
                    }
                    ...
                    if (DBMonitorSnapshot.Instance.LastFreeSpinUpdateTime == new DateTime(1, 1, 1))
                    {
                        DBMonitorSnapshot.Instance.LastFreeSpinUpdateTime = new DateTime(1970, 1, 1);
                        string strQuery = "SELECT TOP 1 updatetime FROM userfreespinevents WHERE updatetime IS NOT null ORDER BY updatetime DESC";
                        SqlCommand command = new SqlCommand(strQuery, connection);
                        using (DbDataReader reader = await command.ExecuteReaderAsync())
                        {
                            if (await reader.ReadAsync())
                            {
                                DBMonitorSnapshot.Instance.LastFreeSpinUpdateTime = reader.GetDateTime(0);
                            }
                        }
                    }
                    if (AgentSnapshot.Instance.LastUpdateTime == new DateTime(1, 1, 1))
                    {
                        ...
                        using (DbDataReader reader = await command.ExecuteReaderAsync())
                        {
                            while (await reader.ReadAsync())
                            {
                                ...
                                bool    promoEnable = (bool)reader["promoenable"];
                                DateTime updateTime = (DateTime)reader["updatetime"];

                                AgentSnapshot.Instance.updateAgentConfig(agentID, new AgentConfig(apiToken, secretKey, callbackURL, promoEnable));
                ...
                using (SqlConnection connection = new SqlConnection(_strConnString))
                {
                    ...
                    using (DbDataReader reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            DBMonitorSnapshot.Instance.LastFreeSpinEventID = (long)reader["id"];

                            if(expireTime > DateTime.UtcNow)
                                Context.System.ActorSelection("/user/userManager").Tell(new UserFreeSpinEventItem(bonusID, agentID, strUserID, gameid, freeSpinCount, betLevel, expireTime, freeSpinID));
                        }
                    }

                    strQuery = "SELECT * FROM userfreespinevents WHERE updatetime > @updatetime  ORDER BY updatetime";
                    command = new SqlCommand(strQuery, connection);
                    command.Parameters.AddWithValue("@updatetime", DBMonitorSnapshot.Instance.LastFreeSpinUpdateTime);
                    using (DbDataReader reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            int gameid          = (int)(reader["gameid"]);
                            int processed       = (int)reader["processed"];
                            DBMonitorSnapshot.Instance.LastFreeSpinUpdateTime = (DateTime)reader["updatetime"];
                            if (processed == 2)
                                Context.System.ActorSelection("/user/userManager").Tell(new UserFreeSpinEventCancelled(agentID, strUserID, gameid, bonusID));
                        }
                    }
