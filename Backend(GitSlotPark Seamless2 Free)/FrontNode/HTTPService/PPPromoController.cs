...
namespace FrontNode.HTTPService
{
    public class PPPromoController : ApiController
    {
        ...
        [HttpPost]
        [Route("gitapi/pp/promo/race/v3/winners")]
        public async Task<HttpResponseMessage> getRaceWinnersV3(RaceWinnerRequestV3 request)
        {
            string strToken = null;
            string strSymbol = null;
            foreach (KeyValuePair<string, string> pair in Request.GetQueryNameValuePairs())
            ...
            if (request == null || request.raceIds == null)
                return new HttpResponseMessage() { Content = new StringContent("unlogged") };
            ...
            GITMessage gitMessage = new GITMessage((ushort)CSMSG_CODE.CS_PP_PROMOV3RACEWINNER);
            gitMessage.Append(request.raceIds.Count);
            foreach (int racId in request.raceIds)
            {
                gitMessage.Append(racId);
                gitMessage.Append("");
            }
        ...
        public class RaceWinnerRequestV3
        {
