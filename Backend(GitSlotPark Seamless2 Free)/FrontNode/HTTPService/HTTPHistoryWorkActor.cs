    ...
    class HTTPHistoryWorkActor : ReceiveActor
    {
        ...
        private async Task onPPHistoryGetItemDetailRequest(HTTPPPHistoryGetItemDetailRequest request)
        {
            try
            {
                ...
                if(request.Version == 2)
                    Sender.Tell(new HttpResponseMessage() { Content = new StringContent(JsonConvert.SerializeObject(historyItems), Encoding.UTF8, "application/json") });

                if(request.Version == 3)
                {
                    PPGameRecentHistoryDetailV3 resContent = new PPGameRecentHistoryDetailV3()
                    {
                        error       = 0,
                        description = "OK",
                        data        = historyItems
                    };
                    Sender.Tell(new HttpResponseMessage() { Content = new StringContent(JsonConvert.SerializeObject(resContent), Encoding.UTF8, "application/json") });
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task onPPHistorySettingRequest(HTTPPPHistoryGenralSettingRequest request)
        {
            try
            {
                ...
                Sender.Tell(new HttpResponseMessage() { Content = new StringContent("{\"language\":\"en\",\"jurisdiction\":\"99\",\"jurisdictionRequirements\":[],\"brandRequirements\":[]}", Encoding.UTF8, "application/json") });
            }
            catch (Exception ex)
            {
