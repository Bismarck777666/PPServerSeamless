...
{
    [RoutePrefix("gitapi/pp/history")]
    public class PPHistoryController : ApiController
    {
        [HttpGet]
        [HttpPost]
        [Route("v2/settings/general")]
        ...
        [HttpGet]
        [HttpPost]
        [Route("v2/play-session/last-items")]
        ...
        public async Task<HttpResponseMessage> getItemDetail ()
        {
            ...
            try
            {
                HttpResponseMessage response = await HTTPServiceConfig.Instance.HistoryWorkerGroup.Ask<HttpResponseMessage>(new HTTPPPHistoryGetItemDetailRequest(strUserID, strToken, gameID, roundID, 2), TimeSpan.FromSeconds(20));
                return response;
            }
            catch (Exception)
            {
        ...
        [HttpGet]
        [HttpPost]
        [Route("v3/action/children")]
        public async Task<HttpResponseMessage> getItemDetailV3 ()
        {
            string strToken     = null;
            string strSymbol    = null;
            string strRoundID   = null;
            ...
            try
            {
                HttpResponseMessage response = await HTTPServiceConfig.Instance.HistoryWorkerGroup.Ask<HttpResponseMessage>(new HTTPPPHistoryGetItemDetailRequest(strUserID, strToken, gameID, roundID, 3), TimeSpan.FromSeconds(20));
                return response;
            }
            catch (Exception)
            {
