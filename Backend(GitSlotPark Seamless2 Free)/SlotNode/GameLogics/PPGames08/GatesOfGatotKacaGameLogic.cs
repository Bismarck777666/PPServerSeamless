...
{
    class GatesOfGatotKacaGameLogic : BasePPSlotGame
    {
        #region 게임고유속성값
        ...
        #endregion
        public GatesOfGatotKacaGameLogic()
        {
            _gameID = GAMEID.GatesOfGatotKaca;
            GameName = "GatesOfGatotKaca";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in GatesOfGatotKacaGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }

                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in GatesOfGatotKacaGameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in GatesOfGatotKacaGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
