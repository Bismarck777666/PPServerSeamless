...
{
    class StarlightChristmasGameLogic : BasePPSlotGame
    {
        #region 게임고유속성값
        ...
        #endregion
        public StarlightChristmasGameLogic()
        {
            _gameID = GAMEID.StarlightChristmas;
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in StarlightChristmasGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }

                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in StarlightChristmasGameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in StarlightChristmasGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
