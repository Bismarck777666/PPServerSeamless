    ...
    class PinupGirlsGameLogic : BasePPSlotGame
    {
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in PinupGirlsGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }
        ...
        protected async Task<BasePPSlotSpinData> selectRandomSpinData(int agentID, BasePPSlotBetInfo betInfo, bool isAffiliate)
        {
            ...
            if(!isAffiliate && !await checkWebsitePayoutRate(agentID, totalBet * 10.0, 10.0 * totalBet * groupSpinData.GroupOdd))
            {
                spinGroupID         = Pcg.Default.Next(1, _emptySpinCount + 1);
