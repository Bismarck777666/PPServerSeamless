    ...
    class AztecBlazeGameLogic : BasePPSlotGame
    {
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in WildWestGoldMegaGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }

                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in WildWestGoldMegaGameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
