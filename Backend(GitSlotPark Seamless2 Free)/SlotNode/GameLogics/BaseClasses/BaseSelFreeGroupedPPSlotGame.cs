...
namespace SlotGamesNode.GameLogics
{
    ...
    {        
        ...
        protected async Task<GroupSequenceSample> pickSequenceSample(int websiteID, BasePPSlotBetInfo baseBetInfo, bool isAffiliate)
        {
            BasePPGroupedSlotBetInfo betInfo = baseBetInfo as BasePPGroupedSlotBetInfo;
            double totalBet = Math.Round(betInfo.TotalBet, 2);
            ...
            if(betInfo.SeqPerBet.ContainsKey(totalBet) && betInfo.SeqPerBet[totalBet].IsEnded && betInfo.SeqPerBet[totalBet].CreditMoney > 0.0 && !isAffiliate)
            ...
            do
            {
                ...
                if (isAffiliate || await checkWebsitePayoutRate(websiteID, 0.0, totalBet * minOdd))
                {
        ...
        protected override async Task<OddAndIDData> selectRandomOddAndID(int websiteID, BasePPSlotBetInfo betInfo, bool isMoreBet, bool isAffiliate)
        {
            ...
            GroupSequenceSample grpSeqSample = await pickSequenceSample(websiteID, betInfo, isAffiliate);
            int selectedID      = 0;
            int affliateCnt     = 0;
            ...
            else
            {
                ...
                else
                {
                    if (_groupNaturalCounts.ContainsKey(groupID) && _groupNaturalCounts[groupID] > 0)
                    {
                        affliateCnt = _groupNaturalCounts[groupID] / 2;
                        selectedID = _groupStartIds[groupID] + affliateCnt + Pcg.Default.Next(0, _groupNaturalCounts[groupID] - affliateCnt);
                    }
                }
            }
        ...
        protected override async Task<BasePPSlotSpinData> selectRandomStop(int websiteID, BasePPSlotBetInfo betInfo, bool isMoreBet, bool isAffiliate)
        {
            GroupOddAndIDData   selectedOddAndID = await selectRandomOddAndID(websiteID, betInfo, isMoreBet, isAffiliate) as GroupOddAndIDData;
        ...
        {
            GroupSequenceSample grpSample   = await pickSequenceSample(websiteID, betInfo, isAffiliate);
        ...
        public override async Task<BasePPSlotSpinData> selectRandomStop(int websiteID, UserBonus userBonus, double baseBet, bool isChangedLineCount, BasePPSlotBetInfo betInfo, bool isAffiliate)
        {
            ...
            if (userBonus != null && userBonus is UserRangeOddEventBonus)
            {
                ...
                {
                BasePPSlotSpinData     spinDataEvent = await selectRangeSpinData(0, rangeOddBonus.MinOdd, rangeOddBonus.MaxOdd, betInfo, isAffiliate);
        ...
        {
            return new BasePPGroupedSlotBetInfo();
        }
    }
