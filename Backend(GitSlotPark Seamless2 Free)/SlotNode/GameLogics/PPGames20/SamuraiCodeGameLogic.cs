...
{
    class SamuraiCodeGameLogic : BasePPSlotGame
    {
        #region 게임고유속성값
        ...
        #endregion
        public SamuraiCodeGameLogic()
        {
            _gameID = GAMEID.SamuraiCode;
            GameName = "SamuraiCode";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in SamuraiCodeGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }
		
                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in SamuraiCodeGameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in SamuraiCodeGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
    }
}
