...
{
    class DragonGold88GameLogic : BasePPSlotGame
    {
        #region 게임고유속성값
        ...
        public DragonGold88GameLogic()
        {
            _gameID     = GAMEID.DragonGold88;
            GameName    = "DragonGold88";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in DragonGold88GameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }
		
                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in DragonGold88GameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in DragonGold88GameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
    }
}
