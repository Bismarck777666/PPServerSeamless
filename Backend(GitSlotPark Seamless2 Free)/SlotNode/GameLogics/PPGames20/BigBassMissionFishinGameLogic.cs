    ...
    class BigBassMissionFishinGameLogic : BaseSelFreePPSlotGame
    {
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in BigBassMissionFishinGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }
		
                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in BigBassMissionFishinGameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
        ...
        protected override void onDoBonus(int agentID, string strUserID, GITMessage message, double userBalance, Currencies currency, bool isAffiliate)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        base.onDoBonus(agentID, strUserID, message, userBalance, currency, isAffiliate);
                        return;
                    }
                    else
                    {
