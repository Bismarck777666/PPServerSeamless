...
{
    class SarayRuyasiGameLogic : BasePPSlotGame
    {
        #region 게임고유속성값
        ...
        public SarayRuyasiGameLogic()
        {
            _gameID     = GAMEID.SarayRuyasi;
            GameName    = "SarayRuyasi";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in SarayRuyasiGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }
		
                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in SarayRuyasiGameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in SarayRuyasiGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
    }
}
