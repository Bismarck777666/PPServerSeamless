    ...
    class SuperXGameLogic : BasePPSlotGame
    {
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in SuperXGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }
        ...
        {
            if (!betInfo.MoreBet)
                return await base.selectEmptySpin(companyID, betInfo, isAffiliate);
        ...
        public override async Task<BasePPSlotSpinData> selectRandomStop(int companyID, UserBonus userBonus, double baseBet, bool isChangedLineCount, BasePPSlotBetInfo betInfo, bool isAffiliate)
        {
            if (!betInfo.MoreBet)
                return await base.selectRandomStop(companyID, userBonus, baseBet, isChangedLineCount, betInfo, isAffiliate);
