...
namespace SlotGamesNode.GameLogics
{
    class DaVinciTreasureGameLogic : BasePPSlotGame
    {
        ...
        protected override async Task onDoGamble(int agentID, string strUserID, GITMessage message, double userBalance, bool isAffiliate)
        {
            try
            {
                int index   = (int)message.Pop();
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (randomIndex == gambleIndex && Pcg.Default.NextDouble(0, 100) <= payoutRate && 
                                (isAffiliate || await checkWebsitePayoutRate(agentID, gambleAmount, boxCount * gambleAmount, 1)))
                            {
                                dicParams["g_l"]   = "1";
