    ...
    class BookOfTheFallenGameLogic : BaseSelFreePPSlotGame
    {
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in BookOfTheFallenGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }
        ...
        {
            return new BookOfTheFallenBetInfo();
        }
        ...
        protected override async Task onProcMessage(string strUserID, int websiteID, GITMessage message, UserBonus userBonus, double userBalance, Currencies currency, bool isAffiliate)
        {
            if (message.MsgCode == (ushort)CSMSG_CODE.CS_PP_DOMYSTERYSCATTER)
                onDoMysteryScatter(strUserID, websiteID, message, userBonus, userBalance, isAffiliate);
            else
                await base.onProcMessage(strUserID, websiteID, message, userBonus, userBalance, currency, isAffiliate);
        }
        protected void onDoMysteryScatter(string strUserID, int agentID, GITMessage message, UserBonus userBonus, double userBalance, bool isAffiliate)
        {
            try
            {
