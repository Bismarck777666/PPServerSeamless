...
{
    class BigBassHalloweenGameLogic : BasePPSlotGame
    {
        #region 게임고유속성값
        ...
        #endregion
        public BigBassHalloweenGameLogic()
        {
            _gameID = GAMEID.BigBassHalloween;
            GameName = "BigBassHalloween";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID, Currencies currency)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in BigBassHalloweenGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }
		
                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in BigBassHalloweenGameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in BigBassHalloweenGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
