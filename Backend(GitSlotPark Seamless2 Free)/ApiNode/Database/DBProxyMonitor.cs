...
namespace QueenApiNode.Database
{
                ...
                using (SqlConnection connection = new SqlConnection(_strConnString))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            bool    isEnabled    = (int)reader["state"] == 1;
                            Context.System.ActorSelection("/user/agentManager").Tell(new AuthTokenUpdated(strAgentID, strAuthToken));
                            Context.System.ActorSelection("/user/agentManager").Tell(new AgentEnabledUpdated(strAgentID, isEnabled));
    ...
    public class AgentEnabledUpdated
    {
        public string AgentID { get; set; }
        public bool IsEnabled { get; set; }
        
        public AgentEnabledUpdated(string agentID, bool isEnabled)
        {
