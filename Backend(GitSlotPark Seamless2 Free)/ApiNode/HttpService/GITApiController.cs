...
namespace QueenApiNode.HttpService
{
    public class GITApiController : ApiController
    {
        ...
        [HttpPost]
        [Route("createfreegame")]
        public async Task<HttpResponseMessage> doCreateFreeGame()
        {
            try
            {
                if (!Request.Headers.Contains("Authorization"))
                ...
                CreateFreeGameRequest request   = JsonConvert.DeserializeObject<CreateFreeGameRequest>(strContent);
                string response                 = await ApiConfig.WorkActorGroup.Ask<string>(new ApiConsistentRequest(authToken, request), TimeSpan.FromSeconds(10.0));
                var responseMessage             = Request.CreateResponse(HttpStatusCode.OK);
                responseMessage.Content         = new StringContent(response, Encoding.UTF8, "application/json");
                return responseMessage;
        ...
        [HttpPost]
        [Route("cancelfreegame")]
        public async Task<HttpResponseMessage> doCancelFreeGame()
        {
            try
            {
                if (!Request.Headers.Contains("Authorization"))
                ...
                CancelFreeGameRequest request   = JsonConvert.DeserializeObject<CancelFreeGameRequest>(strContent);
                string response                 = await ApiConfig.WorkActorGroup.Ask<string>(new ApiConsistentRequest(authToken, request), TimeSpan.FromSeconds(10.0));
                var responseMessage             = Request.CreateResponse(HttpStatusCode.OK);
                responseMessage.Content         = new StringContent(response, Encoding.UTF8, "application/json");
                return responseMessage;
    ...
    public class CreateFreeGameResponse : QueenResponse
    {
        public CreateFreeGameResponse() : base(ResponseCodes.OK, "OK")
        {

        }
    }
    public class CancelFreeGameResponse : QueenResponse
    {
        public CancelFreeGameResponse() : base(ResponseCodes.OK, "OK")
        {
    ...
    public class CreateFreeGameRequest
    {
        ...
        public CreateFreeGameRequest()
        {
        }
    }
    public class CancelFreeGameRequest
    {
