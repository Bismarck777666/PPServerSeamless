...
namespace QueenApiNode.Agent
{
    class AgentManager : ReceiveActor
    {
        ...
        public AgentManager()
        {
            ...
            Receive<AgentEnabledUpdated>(message =>
            {
                var agentActor = Context.Child(message.AgentID);
                if (agentActor != ActorRefs.Nobody)
    ...
    public class CreateNewAgentActorMsg
    {
        ...
        public CreateNewAgentActorMsg(int dbID, string strAgentID, string authToken, int currency, int language, bool isEnabled, IActorRef dbReader, IActorRef dbWriter)
        {
            this.AgentDBID  = dbID;
