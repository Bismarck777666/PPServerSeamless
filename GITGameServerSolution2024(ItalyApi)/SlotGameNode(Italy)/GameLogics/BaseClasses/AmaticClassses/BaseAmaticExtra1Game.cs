...
namespace SlotGamesNode.GameLogics
{
    ...
    class BaseAmaticExtra1Game : BaseAmaticSlotGame
    {
        protected virtual string ExtraString => "8ffffffff";
        protected override string buildInitString(string strGlobalUserID, double balance, CurrencyEnum currency)
        {
            string initString = base.buildInitString(strGlobalUserID, balance, currency);
            BaseAmaticExtraWildInitPacket extraInitPacket = new BaseAmaticExtraWildInitPacket(initString, Cols, FreeCols, ReelSetBitNum, ExtraString);
        ...
        public class BaseAmaticExtraWildPacket : AmaticPacket
        {
            public string extrastr { get; set; }

            public BaseAmaticExtraWildPacket(string message, int reelCnt, int freeReelCnt) : base(message, reelCnt, freeReelCnt)
            {
            ...
            public BaseAmaticExtraWildPacket(int reelCnt, int freeReelCnt, int msgType, int lineCnt, string extraStr) : base(reelCnt, freeReelCnt, msgType, lineCnt)
            {
                extrastr = extraStr;
        ...
        public class BaseAmaticExtraWildInitPacket : InitPacket
        {
            public string extrastr { get; set; }

            public BaseAmaticExtraWildInitPacket(string message, int reelCnt, int freeReelCnt, int reelsetBitCnt, string extraStr) : base(message, reelCnt, freeReelCnt, reelsetBitCnt)
            {
                extrastr    = extraStr;
                curpoint    = curpoint + extrastr.Length;
            }
