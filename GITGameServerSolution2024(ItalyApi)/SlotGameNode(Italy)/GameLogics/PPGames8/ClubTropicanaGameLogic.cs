...
{
    class ClubTropicanaGameLogic : BasePPSlotGame
    {
        #region 게임고유속성값
        ...
        #endregion
        public ClubTropicanaGameLogic()
        {
            _gameID = GAMEID.ClubTropicana;
            GameName = "ClubTropicana";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strGlobalUserID)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in ClubTropicanaGameLogic::readBetInfoFromMessage {1}", strGlobalUserID, betInfo.BetPerLine);
                    return;
                }
		
                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in ClubTropicanaGameLogic::readBetInfoFromMessage", strGlobalUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in ClubTropicanaGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
    }
}
