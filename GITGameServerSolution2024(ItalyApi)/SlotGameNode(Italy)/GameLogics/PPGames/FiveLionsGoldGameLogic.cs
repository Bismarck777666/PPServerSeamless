        ...
        protected override async Task onDoBonus(string strUserID,int agentID, GITMessage message, double userBalance)
        {
            try
            {
                ...
                if (realWin == 0.0)
                    ...
                    Context.System.Scheduler.ScheduleTellOnce(250, Sender, new ToUserResultMessage((int)_gameID, responseMessage, 0.0, realWin, new GameLogInfo(this.GameName, "0", strGameLog),UserBetTypes.Normal),Self);
            }
            catch (Exception ex)
            {
