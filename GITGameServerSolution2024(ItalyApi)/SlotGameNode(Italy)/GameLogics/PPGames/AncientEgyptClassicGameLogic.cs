    ...
    public class AncientEgyptClassicGameLogic : BasePPSlotGame
    {
        ...
        protected override async Task onProcMessage(string strUserID, int agentID, CurrencyEnum currency, GITMessage message, UserBonus userBonus, double userBalance, bool isMustLose)
        {
            ...
            if (message.MsgCode == (ushort)CSMSG_CODE.CS_PP_DOMYSTERYSCATTER)
                onDoMysteryScatter(strGlobalUserID, message, userBonus, userBalance, isMustLose);
            else
                await base.onProcMessage(strUserID, agentID, currency, message, userBonus, userBalance, isMustLose);
        }
        
        protected void onDoMysteryScatter(string strGlobalUserID, GITMessage message, UserBonus userBonus, double userBalance, bool isMustLose)
        {
            try
            {
