...
namespace CommNode
{
    public class AmaticWebsocketListener : ReceiveActor
    {
        ...
        public AmaticWebsocketListener(Config config, IActorRef dbReader, IActorRef dbWriter, IActorRef redisWriter)
        {
            _dbReader       = dbReader;
            _dbWriter       = dbWriter;
        ...
        private async Task processIncomingConnection(HttpListenerContext context)
        {
            try
            {
                ...
                var connection          = Context.ActorOf(Props.Create(() => new AmaticWsClientConnection(websocketContext.WebSocket)));
                var connectionHandler   = Context.ActorOf(AmaticWebsocketClientHandler.Props(connection, remoteEndPoint, _dbReader, _dbWriter, _redisWriter));
                connection.Tell(new AmaticWsClientConnection.RegisterHandler(connectionHandler));
