...
namespace CommNode.Database
{
    public class DBProxyWriter : ReceiveActor
    {
        ...
        public DBProxyWriter(string strConnStriing)
        {
            ...
            Receive<PlayerBalanceUpdateItem>                (updateItem => WriterSnapshot.Instance.updatePlayerBalanceUpdate(updateItem));
            Receive<BaseClaimedUserBonusUpdateItem>         (item       => WriterSnapshot.Instance.claimBonusItem(item));
            Receive<UserStateUpdateItem>                    (updateItem => WriterSnapshot.Instance.updatePlayerState(updateItem));
            Receive<GameLogItem>                            (item       => WriterSnapshot.Instance.insertGameLog(item));
            Receive<ReportUpdateItem>                       (updateItem => ReportSnapshot.Instance.updateReports(updateItem));
            Receive<List<ReportUpdateItem>>                 (message    => ReportSnapshot.Instance.PushReportUpdateItems(message));
            Receive<GameReportItem>                         (updateItem => ReportSnapshot.Instance.updateGameReport(updateItem));
            Receive<List<GameReportItem>>                   (message    => ReportSnapshot.Instance.PushGameReportUpdates(message));
            Receive<List<BaseInsertItem>>                   (message    => WriterSnapshot.Instance.PushDBInsertItems(message));
            Receive<List<BaseClaimedUserBonusUpdateItem>>   (message    => WriterSnapshot.Instance.PushClaimedBonusItems(message));                
            Receive<List<UserStateUpdateItem>>              (message    => WriterSnapshot.Instance.PushPlayerStates(message));
            Receive<List<PlayerBalanceUpdateItem>>          (message    => WriterSnapshot.Instance.PushBalanceUpdateItems(message));
            Receive<List<GameLogItem>>                      (message    => WriterSnapshot.Instance.PushGameLogItems(message));
            Receive<UserBetMoneyUpdateItem>                 (item       => WriterSnapshot.Instance.updateUserBetMoney(item));
            Receive<UserTournamentBetItem>                  (item       => WriterSnapshot.Instance.insertUserTournamentBetItem(item));
            Receive<List<UserTournamentBetItem>>            (message    => WriterSnapshot.Instance.PushUserTournamentBetItems(message));
