...
namespace CommNode
{
    public class WsClientConnection : ReceiveActor
    {
        ...
        public WsClientConnection(WebSocket wsClient)
        {
            ...
            ReceiveAsync<BynaryProtocalWrite>   (onBynaryWriteData);
        ...
        private void onReceiveReadStream(ReadStreamMessage msgReadStream)
        {
            ...
            try
            {
                ...
                else if (msgReadStream.ReadStream.MessageType == WebSocketMessageType.Binary)
                {
                    ...
                    {
                        ms.Write(_recvBuffer, 0, msgReadStream.ReadStream.Count);
                        onReceiveBynaryData(ms.ToArray());
                    }
                }
        ...
        private void onReceiveBynaryData(byte[] receivedData)
        {
        ...
        private async Task onBynaryWriteData(BynaryProtocalWrite writeData)
        {
            if (_wsClient.State != WebSocketState.Open)
            {
                Self.Tell("disconnected");
            ...
            try
            {
                await _wsClient.SendAsync(new ArraySegment<byte>(writeData.Data), WebSocketMessageType.Binary, true, CancellationToken.None);
            }
            catch(Exception ex)
            {
                //웹소켓에서 자료를 읽는 과정에 례외발생
        ...
        public class BynaryProtocalReceived
        {
            public byte[] ReceivedData { get; private set; }
            public BynaryProtocalReceived(byte[] receivedData)
            {
                this.ReceivedData = receivedData;
            }
        }
        
        public class BynaryProtocalWrite
        {
            public byte[] Data { get; private set; }

            public BynaryProtocalWrite(byte[] data)
            {
                this.Data = data;
            }
        }
