...
namespace CommNode
{
    public class WebsocketListener : ReceiveActor
    {
        ...
        public WebsocketListener(Config config, IActorRef dbReader, IActorRef dbWriter, IActorRef redisWriter)
        {
            _dbReader       = dbReader;
            _dbWriter       = dbWriter;
        ...
        private void processCommand(string strCommand)
        {
            if (strCommand == "start")
            {
                ...
                {
                    _httpListener.Start();
                    _log.Info("Starting Websocket service for url {0}", _listenURL);
                }
                catch (Exception ex)
                {
                    _log.Error("Failed in starting Websocket service: Exception {0}", ex.Message);
                    return;
                }

                //Incoming 련결들을 접수한다.
                _log.Info("Listening websocket service....");
        ...
        private async Task processIncomingConnection(HttpListenerContext context)
        {
            try
            {
                ...
                var connectionHandler = Context.ActorOf(WebsocketClientHandler.Props(connection, remoteEndPoint, _dbReader, _dbWriter, _redisWriter));
                connection.Tell(new WsClientConnection.RegisterHandler(connectionHandler));

                _log.Info("Websocket Connect request from {0}", remoteEndPoint.ToString());
            }
            catch (Exception ex)
            {
                _log.Error("Exception has been occurred in WebsocketListener::processIncomingConnection {0}", (object)ex);
