    ...
    public class PPReplayController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> makeReplayLink()
        {
            try
            {
                ...
                try
                {
                    string strResponse = await HTTPServiceConfig.Instance.ReplayWorkerGroup.Ask<string>(new HTTPPPReplayMakeLinkRequest(strUserID, strToken, gameID, roundID, strSymbol, envID, lang), TimeSpan.FromSeconds(20));
                    return new HttpResponseMessage() { Content = new StringContent(strResponse) };
                }
                catch (Exception)
                {
