...
namespace CommNode.HTTPService
{
    ...
    public class PPHistoryController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> getItemDetail ()
        {
            ...
            string strGlobalUserID = findUserIDFromToken(strToken);
            ...
            try
            {
                HttpResponseMessage response = await HTTPServiceConfig.Instance.HistoryWorkerGroup.Ask<HttpResponseMessage>(new HTTPPPHistoryGetItemDetailRequest(strGlobalUserID, strToken, gameID, strRoundID), TimeSpan.FromSeconds(20));
                return response;
            }
            catch (Exception)
            {
