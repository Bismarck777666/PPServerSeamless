...
namespace SlotGamesNode
{
    public class BootstrapActor : ReceiveActor
    {
        ...
        private async Task onReadyDBProxy(DBProxy.ReadyDBProxy dbActors)
        {
            ...
            JackpotPoolConfig.Instance.PoolActor = Context.System.ActorOf(Akka.Actor.Props.Create(() => new JackpotPoolActor()), "jackpotPool");
            Context.System.ActorOf(Akka.Actor.Props.Create(() => new SpinDBReader(_configuration.GetConfig("spindb"))).WithRouter(FromConfig.Instance), "spinDBReaders");
