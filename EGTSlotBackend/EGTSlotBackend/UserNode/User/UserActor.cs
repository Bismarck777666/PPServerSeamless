...
namespace UserNode
{
    public class UserActor : ReceiveActor
    {
        ...
        public UserActor(CreateNewUserMessage message, IActorRef dbReader, IActorRef dbWriter)
        {
            ...
            ReceiveAsync<FromHttpRevMessage>            (onProcHttpMessage);
        ...
        private async Task onClosedUserConnection(UserConnection userConn)
        {
            ...
            if (userConn.GameActor != null)
            {
                ...
                try
                {
                    ...
                    if (response is EGTExitResponse)
                        await procEGTExitGameResponse(response as EGTExitResponse);
                }
                catch (Exception ex)
                {
        ...
        private async Task onProcMessage(FromConnRevMessage fromConnRevMsg)
        {
            ...
            if (message.MsgCode == (ushort)CSMSG_CODE.CS_HEARTBEAT)
            {
                ...
                JObject pingResponse = new JObject
                {
                ...
                GITMessage pingMsg = new GITMessage((ushort)SCMSG_CODE.SC_HEARTBEAT);
                pingMsg.Append(pingResponse.ToString());
                Sender.Tell(new SendMessageToUser(pingMsg, _balance, 0.0));
            }
            ...
            else if (message.MsgCode >= (ushort)CSMSG_CODE.CS_EGT_SLOTGAMESTART && message.MsgCode <= (ushort)CSMSG_CODE.CS_EGT_SLOTGAMEEND)
            {
                userConn.LastActiveTime = DateTime.Now;
        ...
        private async Task procEGTExitGameResponse(EGTExitResponse response)
        {
            try
            {
            ...
            {
                _logger.Error("Exception has been occurred in UserActor::procEGTExitGameResponse {0}", ex);
            }
        }

        private async Task onProcHttpMessage(FromHttpRevMessage fromHttpRevMsg)
        {
            if (_userDisconnected)
            {
            ...
            GITMessage message = fromHttpRevMsg.Message;
            int gameID = int.Parse(message.Pop().ToString());
            UserConnection userConn = _userConnections.Values.FirstOrDefault(conn => conn.GameID == gameID);
            if (userConn == null)
            {
                Sender.Tell("invalidaction");
                return;
        ...
        private async Task procToUserMessage(ToUserMessage message, GITMessage gameMessage, UserConnection userConn)
        {
            if (message is ToUserResultMessage)
            {
                ...
                if((gameMessage.MsgCode == (ushort) CSMSG_CODE.CS_EGT_DOSPIN || gameMessage.MsgCode == (ushort)CSMSG_CODE.CS_EGT_DOGAMBLEHALF || gameMessage.MsgCode == (ushort)CSMSG_CODE.CS_EGT_DOGAMBLE))
                {
                    if(!isSuccess)
                    {
                        GITMessage unauthorizationMsg = new GITMessage((ushort)SCMSG_CODE.SC_EGT_DOSPIN);
                        unauthorizationMsg.Append("-1unauthorization");
                        Sender.Tell(new SendMessageToUser(unauthorizationMsg, _balance, 0.0));

                        GITMessage errorResultMsg = new GITMessage((ushort)CSMSG_CODE.CS_EGT_NOTPROCDRESULT);
