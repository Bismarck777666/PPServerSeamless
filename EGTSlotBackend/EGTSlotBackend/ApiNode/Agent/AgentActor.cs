    ...
    class AgentActor : ReceiveActor
    {
        ...
        private void onGetGameList(GetGameListRequest request)
        {
            try
            {
                ...
                foreach(GameInfo gameInfo in gameInfos)
                {
                    ...
                    apiGame.vendorid = "EGT";
                    apiGame.gameid   = gameInfo.GameID;
                    apiGame.symbol   = gameInfo.Symbol;
                    apiGame.iconurl1 = string.Format("{0}/{1}.webp", ApiConfig.GameFrontImageUrl, gameInfo.Symbol);
                    apiGame.iconurl2 = string.Format("{0}/{1}.webp", ApiConfig.GameFrontImageUrl, gameInfo.Symbol);
        ...
        private async Task onUserAuthRequest(UserAuthRequest request)
        {
            try
            {
                ...
                string strGameURL = string.Format("{0}/egt/openGame?agentid={1}&userid={2}&token={3}&symbol={4}&currency={5}&lang={6}&lobbyurl={7}",
                ...
                //string strGameURL = string.Format("{0}/?agentid={1}&userid={2}&token={3}&symbol={4}&currency={5}&lang={6}&lobbyurl={7}",
                //    ApiConfig.GameFrontUrl, _dbID, strUserID, encryptString(token, ApiConfig.FrontTokenKey), gameInfo.Symbol, _currency, request.lang, request.lobbyUrl);
