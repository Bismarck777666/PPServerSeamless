...
namespace FrontNode
{
    public class BootstrapActor : ReceiveActor
    {
        ...
        private IActorRef                   _webSocketListener          = null; // EGT WebSocket
        private IActorRef                   _httpListener               = null; // EGT Http
        ...
        public BootstrapActor(Config configuration)
        {
            ...
            Receive<DBProxy.ReadyDBProxy>(dbActors =>
            {
                ...
                if (httpConfig != null)
                {
                    _httpListener = Context.System.ActorOf(Akka.Actor.Props.Create(() => new HttpListener(httpConfig, dbActors.Reader)), "httpListener");
                    _httpListener.Tell("start");
                }
