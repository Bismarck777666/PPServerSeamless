...
namespace UserNode
{
            ...
            try
            {
                ...
                foreach (var item in _retryTasks)
                {
                    ...
                    if (item is RollbackRequestWithRetry)
                    {
                        ...
                        var response = await onRollback(request);
                        if (response == null)
                        {
                            request.RetryCount--;
                            if (request.RetryCount < 0)
                            {
                                ...
                                _dbWriter.Tell(new FailedTransactionItem(request.AgentID, request.UserID, TransactionTypes.Rollback, request.TransactionID, "", false, request.BetAmount, request.WinAmount, request.GameID, DateTime.UtcNow));
                            }
                        }
                    ...
                    else
                    {
                        ...
                        if (response == null)
                        {
                            ...
                            {
                                toDelItems.Add(request);
                                _dbWriter.Tell(new FailedTransactionItem(request.AgentID, request.UserID, TransactionTypes.Deposit, request.TransactionID, request.RefTransactionID, request.EndTransaction, 0.0, request.Amount, request.GameID, DateTime.UtcNow));
                            }
                        }
    ...
    public class DepositRequestWithRetry : CallbackApiRequestWithRetry
    {
        ...
        public DepositRequestWithRetry(string apiURL, string agentID, string userID, string secretKey, int gameID, int retryCount, DateTime lastTriedTime, string transactionID, string refTransactionID, string roundID, bool endTransaction, double amount) :
            base(apiURL, agentID, userID, secretKey, gameID, retryCount, lastTriedTime)
        {
            this.TransactionID      = transactionID;
