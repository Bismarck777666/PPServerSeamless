...
namespace UserNode.Database
{
    public class DBProxyWriter : ReceiveActor
    {
        ...
        public DBProxyWriter(string strConnStriing)
        {
            ...
            Receive<List<PGGameHistoryDBItem>>(_ => 
            { 
                WriterSnapshot.Instance.PushPGGameHistoryItems(_); 
            });
            Receive<PGGameHistoryDBItem>(_ => 
            { 
                WriterSnapshot.Instance.insertPGGameHistory(_); 
            });
