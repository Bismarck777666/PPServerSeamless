    ...
    public class DBGameLogWriteWorker : ReceiveActor
    {
                ...
                do
                {
                    try
                    {
                        using (SqlConnection connection = new SqlConnection(_strConnectionString))
                        {
                            ...
                            int pgHistoryCount       = await insertPGGameHistory(connection);

                            if (insertedLogCount == 0 && transactionCount == 0 && faildTransationCount == 0 && updateTransCount == 0 && pgHistoryCount == 0)
                                break;
                        }
                    }
                    catch (Exception ex)
        ...
        private async Task<int> insertApiTransactions(SqlConnection connection)
        {
            ...
            try
            {
                ...
                insertDataTable.Columns.Add("endtransaction",           typeof(int));
                ...
                foreach (ApiTransactionItem item in transactionItems)
                {
                    insertDataTable.Rows.Add(item.AgentID, item.UserID, item.TransactionID, item.RelTransactionID, item.PlatformTransID, 
                        (int)item.TransactionType, item.EndTransaction ? 1 : 0,  item.GameID, (decimal)item.BetAmount, (decimal)item.WinAmount, item.Timestamp);
                }
        ...
        private async Task<int> insertFailedTransactions(SqlConnection connection)
        {
            ...
            try
            {
                ...
                dataTable.Columns.Add("endtransaction",     typeof(int));
                ...
                foreach (var item in insertItems)
                    dataTable.Rows.Add(item.AgentID, item.UserID, (int) item.TransactionType, item.TransactionID, item.RefTransactionID, item.EndTransaction ? 1 : 0,
                        (decimal) Math.Round(item.BetAmount, 2), (decimal)Math.Round(item.WinAmount, 2), item.GameID, item.UpdateTime);
            ...
            foreach (KeyValuePair<int, List<GameLogItem>> pair in dicApiGameLogItems)
            {
                ...
                try
                {
                    ...
                    foreach (GameLogItem item in pair.Value)
                        dataTable.Rows.Add(item.UserID, item.GameID, item.GameName, item.BetType.ToString(), (decimal)Math.Round(item.Bet, 2), (decimal)Math.Round(item.Win, 2), (decimal)Math.Round(item.BeginMoney, 2), (decimal)Math.Round(item.EndMoney, 2), item.GameLog, item.LogTime);
        ...
        private async Task<int> insertPGGameHistory(SqlConnection connection)
        {
            ...
            try
            {
                ...
                foreach (PGGameHistoryDBItem item in historyItems)
                    dataTable.Rows.Add(item.AgentID, item.UserID, item.GameID, item.Bet, item.Profit, item.Timestamp, item.TransactionID, item.Data);
