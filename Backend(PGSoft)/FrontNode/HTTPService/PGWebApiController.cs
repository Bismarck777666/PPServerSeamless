...
[assembly: OwinStartup(typeof(FrontNode.HTTPService.PGWebApiController))]
namespace FrontNode.HTTPService
{
    ...
    public class PGWebApiController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> verifyOperatorSession([FromUri] string traceId, [FromBody] VerifyOperatorSessionRequest request )
        {
            ...
            try
            {
                string  decryptedOS = decryptString(System.Web.HttpServerUtility.UrlTokenDecode(request.os), ApiConfig.FrontTokenKey);
                int     agentID     = 0;
                string  strUserID   = "";
                string  strPassword = "";
                if (!splitUserIDAndPassword(decryptedOS, ref agentID, ref strUserID, ref strPassword))
                    return Request.CreateResponse(HttpStatusCode.OK, new VerifySessionResponse(1700, "OERR: Operator return an error. Failed to verify operator player session", traceId), Configuration.Formatters.JsonFormatter);

                HTTVerifyResponse response = await HTTPServiceConfig.Instance.AuthWorkerGroup.Ask<HTTVerifyResponse>(new HTTPOperatorVerifyRequest(agentID, strUserID, strPassword, request.gi),
                ...
                if (response.Result == HttpVerifyResults.OK)
                {
                    ...
                    verifyResponse.dt.setCurrency(response.Currency);
                    verifyResponse.dt.uiogc.gc          = 1;
                    verifyResponse.dt.uiogc.ign         = 1;
        ...
        public async Task<HttpResponseMessage> verifySession([FromUri] string traceId, [FromBody] VerifySessionRequest request)
        {
            ...
            try
            {
                ...
                if (!findUserIDFromToken(request.tk, ref agentID, ref strUserID))
                    return Request.CreateResponse(HttpStatusCode.OK, new VerifySessionResponse(1700, "OERR: Operator return an error. Failed to verify operator player session", traceId), Configuration.Formatters.JsonFormatter);

                HTTVerifyResponse response = await HTTPServiceConfig.Instance.AuthWorkerGroup.Ask<HTTVerifyResponse>(new HTTPVerifyRequest(agentID, strUserID, request.tk, request.gi),
                                                TimeSpan.FromSeconds(10));
        ...
        public async Task<HttpResponseMessage> doGetGameRule([FromUri] string traceId, [FromBody] GetGameRuleRequest request)
        {
            try
            {
                ...
                object response = await HTTPServiceConfig.Instance.WorkerGroup.Ask<object>(new FromHTTPSessionMessage(agentID, strUserID, request.atk, message), TimeSpan.FromSeconds(20));
                if (response is ToHTTPSessionMsgResults)
                    return Request.CreateResponse(HttpStatusCode.OK, new GetGameRuleResponse(1302, "Invalid player session", traceId), Configuration.Formatters.JsonFormatter);
                else
                    return Request.CreateResponse(HttpStatusCode.OK, JsonConvert.DeserializeObject<GetGameRuleResponse>(response as string), Configuration.Formatters.JsonFormatter);
            }
            catch (Exception)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new GetGameRuleResponse(1302, "Invalid player session", traceId), Configuration.Formatters.JsonFormatter);
        ...
        public async Task<HttpResponseMessage> doGetGameWallet([FromUri] string traceId, [FromBody] GetGameWalletRequest request)
        {
            try
            {
                ...
                if (response is ToHTTPSessionMsgResults)
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK, convert(response as GetGameWalletResponseData), Configuration.Formatters.JsonFormatter);
            }
            catch (Exception)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new GetGameWalletResponse(1302, "Invalid player session", traceId), Configuration.Formatters.JsonFormatter);
        ...
        public async Task<HttpResponseMessage> doGetyResourcesTypeIds([FromUri] string traceId, [FromBody] GetByResourcesRequest request)
        {
            try
            {
                ...
                if(response is ToHTTPSessionMsgResults)
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK, convert(response as GetByResourcesResponseData), Configuration.Formatters.JsonFormatter);
            }
            catch (Exception)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new GetByResourcesResponse(1302, "Invalid player session", traceId), Configuration.Formatters.JsonFormatter);
        ...
        public async Task<HttpResponseMessage> doGetBetHistory([FromUri] string traceId, [FromBody] GetBetHistoryRequest request)
        {
            try
            {
                ...
                {
                    var ret = JsonConvert.DeserializeObject<GetBetHistoryResponse>((response as GetBetHistoryResponseMessage).Data);
                    return Request.CreateResponse(HttpStatusCode.OK, ret, Configuration.Formatters.JsonFormatter);
                }
            }
            catch (Exception)
            {
        ...
        [HttpPost]
        //[Route("game-proxy/v1v1/BetDetails/Get")]
        [Route("game-proxy/v1/BetDetails/Get")]
        public async Task<HttpResponseMessage> doGetBetDetails([FromBody] GetBetDetailsRequest request)
        {
            try
            {
                if (string.IsNullOrEmpty(request.atk))
                    return Request.CreateResponse(HttpStatusCode.OK, new GetBetHistoryResponse(1302, "Invalid player session", null), Configuration.Formatters.JsonFormatter);
                ...
                GITMessage message = new GITMessage(MsgCodes.GETHISTORYDETAIL);
                message.Append(request.gid);
                message.Append(request.sid);

                object response = await HTTPServiceConfig.Instance.WorkerGroup.Ask<object>(new FromHTTPSessionMessage(agentID, strUserID, request.atk, message), TimeSpan.FromSeconds(20));
                if (response is ToHTTPSessionMsgResults)
                {
                    return Request.CreateResponse(HttpStatusCode.OK, new GetBetHistoryResponse(1302, "Invalid player session", request.atk), Configuration.Formatters.JsonFormatter);
                }
                else
                {
        ...
        private bool splitUserIDAndPassword(string strToken, ref int agentID, ref string strUserID, ref string strPassword)
        {
            ...
            DateTime expireTime;
            if(!DateTime.TryParse(strParts[3], out expireTime) || expireTime < DateTime.UtcNow)
                return false;

            agentID     = int.Parse(strParts[0]);
            strUserID   = strParts[1];
            strPassword = strParts[2];
            return true;
        }
        ...
        private GetByResourcesResponse convert(GetByResourcesResponseData data)
        {
            GetByResourcesResponse response = new GetByResourcesResponse();
            response.dt = JsonConvert.DeserializeObject<dynamic>(data.dt);
            response.err = data.err;
            return response;
        }
        private GetGameWalletResponse convert(GetGameWalletResponseData data)
        {
            GetGameWalletResponse response = new GetGameWalletResponse();
            response.dt                    = JToken.Parse(data.dt);
            response.err                    = data.err;
            return response;
        }
    ...
    public class GetBetDetailsRequest
    {
        public int      gid { get; set; }   //gameid
        public long     sid { get; set; }   //spin roundid
        public string   atk { get; set; }
        public int      pf  { get; set; }
        public string   btt { get; set; }
    }
    ...
    public class VerifySessionResponseData
    {
        ...
        public string                   eatk    { get; set; }
        public JCComponent              jc      { get; set; }
        ...
        public void setCurrency(string strCurrency)
        {
            ...
            else if (strCurrency == "THB")
            {
                this.cc = "THB";
                this.cs = "฿";
            }
            else if (strCurrency == "BRL")
            {
                this.cc = "BRL";
                this.cs = "R$";
            }
            else if(strCurrency == "TND")
            {
                this.cc = "TND";
                this.cs = "د.ت";
            }
            ...
            else if(strCurrency == "RT")
            {
                this.cc = "RT";
                this.cs = "RT";
            }
            else if (strCurrency == "JC")
            {
                this.cc = "JC";
                this.cs = "JC";
            }
            else if (strCurrency == "GC")
            {
                this.cc = "GC";
                this.cs = "GC";
            }
            else if (strCurrency == "CHF")
            {
                this.cc = "CHF";
                this.cs = "CHF";
            }
            else if (strCurrency == "MAD")
            {
                this.cc = "MAD";
                this.cs = "DH";
            }
            else if (strCurrency == "AED")
            {
                this.cc = "AED";
                this.cs = "AED";
            }
            else if (strCurrency == "SYP")
            {
                this.cc = "SYP";
                this.cs = "£";
            }
            else if (strCurrency == "IQD")
            {
                this.cc = "IQD";
                this.cs = "ID";
            }
            else if (strCurrency == "ARS")
            {
                this.cc = "ARS";
                this.cs = "$";
            }
            else if (strCurrency == "EGP")
            {
                this.cc = "EGP";
                this.cs = "£";
            }
            else if (strCurrency == "BWP")
            {
                this.cc = "BWP";
                this.cs = "BWP";
            }
            else if (strCurrency == "ZAR")
            {
                this.cc = "ZAR";
                this.cs = "R";
            }
            else if (strCurrency == "RUB")
            {
                this.cc = "RUB";
                this.cs = "₽";
            }
            else if (strCurrency == "UAH")
            {
                this.cc = "UAH";
                this.cs = "₴";
            }
            else if (strCurrency == "NZD")
            {
                this.cc = "NZD";
                this.cs = "$";
            }
            else if (strCurrency == "AUD")
            {
                this.cc = "AUD";
                this.cs = "$";
            }
            else if (strCurrency == "CAD")
            {
                this.cc = "CAD";
                this.cs = "$";
            }
            else if (strCurrency == "DKK")
            {
                this.cc = "DKK";
                this.cs = "Kr";
            }
            else if (strCurrency == "SEK")
            {
                this.cc = "SEK";
                this.cs = "Kr";
            }
            else if (strCurrency == "RON")
            {
                this.cc = "RON";
                this.cs = "RON";
            }
            ...
            else if (strCurrency == "NAD")
            {
                this.cc = "NAD";
                this.cs = "N$";
            }
            else if (strCurrency == "GHS")
            {
                this.cc = "GHS";
                this.cs = "GHS";
            }
            else if (strCurrency == "PEN")
            {
                this.cc = "PEN";
                this.cs = "PEN";
            }
            else if (strCurrency == "SZL")
            {
                this.cc = "SZL";
                this.cs = "pl";
            }
            else if (strCurrency == "MXN")
            {
                this.cc = "MXN";
                this.cs = "MX$";
            }
            else if (strCurrency == "HKD")
            {
                this.cc = "HKD";
                this.cs = "HK$";
            }
            else if (strCurrency == "SGD")
            {
                this.cc = "SGD";
                this.cs = "S$";
            }
            else if (strCurrency == "PHP")
            {
                this.cc = "PHP";
                this.cs = "₱";
            }
            else if (strCurrency == "AZN")
            {
                this.cc = "AZN";
                this.cs = "₼";
            }
            else if (strCurrency == "GBP")
            {
                this.cc = "GBP";
                this.cs = "£";
            }
            else if (strCurrency == "BOB")
            {
                this.cc = "BOB";
                this.cs = "Bs";
            }
            else if (strCurrency == "GEL")
            {
                this.cc = "GEL";
                this.cs = "₾";
            }
            else if (strCurrency == "KWD")
            {
                this.cc = "KWD";
                this.cs = "KWD";
            }
            else if (strCurrency == "PLN")
            {
                this.cc = "PLN";
                this.cs = "PLN";
            }
            else if (strCurrency == "TMT")
            {
                this.cc = "TMT";
                this.cs = "TMT";
            }
            else if (strCurrency == "TVD")
            {
                this.cc = "TVD";
                this.cs = "TV$";
            }
            else if (strCurrency == "BAM")
            {
                this.cc = "BAM";
                this.cs = "BAM";
            }
            else if (strCurrency == "LBP")
            {
                this.cc = "LBP";
                this.cs = "L£";
            }
            else if (strCurrency == "MNT")
            {
                this.cc = "MNT";
                this.cs = "₮";
            }
            ...
            else if (strCurrency == "JPY")
            {
                this.cc = "JPY";
                this.cs = "¥";
            }
            else if (strCurrency == "MMK")
            {
                this.cc = "MMK";
                this.cs = "Ks";
            }
            else if (strCurrency == "KRW")
            {
                this.cc = "KRW";
                this.cs = "₩";
            }
            else if (strCurrency == "VND")
            {
                this.cc = "VND";
                this.cs = "₫";
            }
            else if (strCurrency == "LKR")
            {
                this.cc = "LKR";
                this.cs = "Rs";
            }
            else if (strCurrency == "KZT")
            {
                this.cc = "KZT";
                this.cs = "₸";
            }
            else if (strCurrency == "PKR")
            {
                this.cc = "PKR";
                this.cs = "Rs";
            }
            else if (strCurrency == "KES")
            {
                this.cc = "KES";
                this.cs = "KES";
            }
            else if (strCurrency == "BYN")
            {
                this.cc = "BYN";
                this.cs = "Br";
            }
    ...
    public class UIOperatorGameComponents
    {
        ...
        */
        ...
        public UIOperatorGameComponents()
        {
            ...
            this.rp     = 0;
            this.gc     = 0;
            this.ign    = 0; //1: show game name, 0: hide
            this.tsn    = 0;
            this.we     = 0;
            this.gsc    = 0;
            this.bu     = 0;
    ...
    public class JCComponent
    {
        ...
        public int hnp      { get; set; }
        public int ts       { get; set; }
        public int smpo     { get; set; }
        public int swf      { get; set; }
