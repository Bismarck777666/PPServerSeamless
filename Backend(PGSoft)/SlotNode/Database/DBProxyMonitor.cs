        ...
        private async Task initializeMonitor()
        {
            using (SqlConnection connection = new SqlConnection(_strConnString))
            {
                ...
                if (DBMonitorSnapshot.Instance.GameConfigUpdateTime == new DateTime(1, 1, 1))
                {
                    ...
                    string strQuery         = "SELECT gameid, gamesymbol, payoutrate, updatetime FROM gameconfigs order by updatetime";
        ...
        private async Task monitorTables()
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(_strConnString))
                {
                    ...
                    //싱글게임설정을 감시한다.
                    string strQuery = "SELECT gameid, payoutrate, updatetime FROM gameconfigs WHERE updatetime > @updatetime ORDER BY updatetime";
                    SqlCommand command = new SqlCommand(strQuery, connection);
                    ...
                    using (DbDataReader reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            if (!PayoutConfigSnapshot.Instance.PayoutConfigs.ContainsKey(gameID))
                                ...
                                PayoutConfigSnapshot.Instance.PayoutConfigs[gameID].PayoutRate = payoutrate;
