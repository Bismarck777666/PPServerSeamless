...
namespace QueenApiNode.Database
{
                ...
                using (SqlConnection connection = new SqlConnection(_strConnString))
                {
                    ...
                    using (var reader = await command.ExecuteReaderAsync())
                    {
                        while (await reader.ReadAsync())
                        {
                            ...
                            Context.System.ActorSelection("/user/agentManager").Tell(new AuthTokenUpdated(strAgentID, strAuthToken, state));
                            DBMonitorSnapshot.Instance.LastAgentUpdateTime = (DateTime)reader["updatetime"];
                        }
                    }
    ...
    public class AuthTokenUpdated
    {
        ...
        public AuthTokenUpdated(string agentID, string authToken, int state)
        {
            AgentID   = agentID;
            AuthToken = authToken;
            State     = state;
