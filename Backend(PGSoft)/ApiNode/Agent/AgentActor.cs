...
namespace QueenApiNode.Agent
{
    class AgentActor : ReceiveActor
    {
        ...
        public AgentActor(CreateNewAgentActorMsg message)
        {
            ...
            Receive<StateUpdated>               (onStateUpdated);
        ...
        private void onGetGameList(GetGameListRequest request)
        {
            try
            {
                ...
                foreach(GameInfo gameInfo in gameInfos)
                {
                    ...
                    apiGame.vendorid = "PG Soft";
                    apiGame.gameid   = gameInfo.GameID;
                    apiGame.symbol   = gameInfo.Symbol;
                    apiGame.iconurl  = string.Format("https://static.pgf-egw0zz.com/icons/{0}.jpg", gameInfo.GameID);
        ...
        private async Task onUserAuthRequest(UserAuthRequest request)
        {
            try
            {
                ...
                string lang          = string.IsNullOrEmpty(request.lang) ? "en" : request.lang;
                string token         = string.Format("{0}_{1}_{2}_{3}", _dbID, strUserID, strPassword, DateTime.UtcNow.AddMinutes(5.0).ToString("yyyy-MM-dd HH:mm:ss"));                
                string strGameURL    = string.Format("https://{1}/{2}/index.html?l={3}&btt=1&ot={4}&ops={5}&f={6}&or={7}",
                        ApiConfig.GameFrontProtocol, ApiConfig.GameFrontDomain, request.gameid, lang, ApiConfig.OperatorGUID, encryptString(token, ApiConfig.FrontTokenKey),
                        Uri.EscapeDataString(request.lobbyUrl), ApiConfig.OriginParam);
        ...
        private void onStateUpdated(StateUpdated updated)
        {
            if (updated.State == 0)
                _isEnabled = false;
            else
