    ...
    public class BootstrapActor : ReceiveActor
    {
        ...
        public BootstrapActor(Config configuration)
        {
            ...
            Receive<DBProxy.ReadyDBProxy>(dbActors =>
            {
                ...
                Context.System.ActorOf(Akka.Actor.Props.Empty.WithRouter(FromConfig.Instance), "commServers");
                Context.System.ActorOf(Akka.Actor.Props.Empty.WithRouter(FromConfig.Instance), "commAuthWorkers");
                ...
                var domainConfig = _configuration.GetConfig("domaininfo");

                if (domainConfig == null) {
                    _logger.Error("config.hocon doesn't contain domain configuration");
                    return;
                }

                ApiConfig.Domain        = domainConfig.GetString("domain",  "");
                ApiConfig.SUbAdvent     = domainConfig.GetString("advent",  "");
                ApiConfig.SubClientB    = domainConfig.GetString("clientb", "");
                ApiConfig.SubGame       = domainConfig.GetString("game",    "");
                ApiConfig.Local         = domainConfig.GetString("local", "");
            });
