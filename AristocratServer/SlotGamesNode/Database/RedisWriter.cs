...
namespace SlotGamesNode.Database
{
    public class RedisWriter : ReceiveActor
    {
        ...
        private async Task onUserSettingWrite(UserSettingWrite writeInfo)
        {
            try
            {
                string strKey = string.Format("{0}_{1}_setting", writeInfo.UserID, writeInfo.GameID);
        ...
        private async Task onUserHistoryWrite(UserHistoryWrite writeInfo)
        {
            try
            {
                string strKey = string.Format("{0}_{1}_history", writeInfo.UserID, writeInfo.GameID);
        ...
        private async Task onUserFreeSpinInfoWrite(UserFreeSpinInfoWrite writeInfo)
        {
            try
            {
                string strKey = string.Format("{0}_{1}_freespin", writeInfo.GlobalUserID, (int)writeInfo.GameID);
            ...
            {
                _logger.Error("Exception has been occurred in RedisWriter:￼nUserFreeSpinInfoWrite {0}", ex);
            }
            if (writeInfo.IsWait)
                Sender.Tell(true);
        }
    ...
    public class UserHistoryWrite : IConsistentHashable
    {
        ...
        public UserHistoryWrite(string strUserID, GAMEID gameID, byte[] historyData, bool isWait)
        {
            this.UserID = strUserID;
            this.GameID = gameID;
    ...
    public class UserSettingWrite : IConsistentHashable
    {
        ...
        public UserSettingWrite(string strUserID, GAMEID gameID, string strSetting, bool isWait)
        {
            this.UserID = strUserID;
            this.GameID = gameID;
