        ...
        private async Task initializeMonitor()
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(_strConnString))
                {
                    ...
                    strQuery = "SELECT * FROM gameconfigs";
                    ...
                    using (DbDataReader reader = await command.ExecuteReaderAsync())
                    {
                        ...
                        while (await reader.ReadAsync())
                        {
                            ...
                            string          coverImg        = (string)              reader["coverimg"];
                            double          weekturnover    = (double)(decimal)     reader["weekturnover"];
                            double          monthturnover   = (double)(decimal)     reader["monthturnover"];
                            DateTime        releasedate     = (DateTime)            reader["releasedate"];

                            DBMonitorSnapshot.Instance.setGameInfo(gameID, gameType, gameSymbol, gameName, coverImg, title, monthturnover, weekturnover, releasedate);
        ...
        private async Task monitorTables()
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(_strConnString))
                {
                    ...
                    string  strQuery = "SELECT username, apitoken, updatetime FROM agents WHERE (apitoken is not null) and (updatetime > @updatetime) ORDER BY updatetime";
                    var     command  = new SqlCommand(strQuery, connection);
                    ...
                    strQuery = "SELECT * FROM gameconfigs WHERE updatetime > @updatetime ORDER BY updatetime";
                    command = new SqlCommand(strQuery, connection);
