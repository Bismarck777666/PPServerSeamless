    ...
    public class HTTPWorkActor : ReceiveActor
    {
        ...
        private async Task onEnterGame(HTTPEnterGameRequest request)
        {
            try
            {
                ...
                int gameID = int.Parse(request.GameIdentifier);
                if (gameID == 0)
                {
                ...
                await sendEnterRequestToUserActor(request.UserID, gameID, strUserActorPath, request.SessionToken, request.IPAddress);
            }
            catch (Exception ex)
            {
                Sender.Tell(HTTPEnterGameResults.INVALIDTOKEN);
        ...
        private async Task sendEnterRequestToUserActor(string strUserID, int gameID, string strUserActorPath, string strSessionToken, string strIPAddress)
        {
            try
            {
                GITMessage gitMessage = new GITMessage((ushort)CSMSG_CODE.CS_ENTERGAME);
                gitMessage.Append((ushort)gameID);
                gitMessage.Append(strIPAddress);
