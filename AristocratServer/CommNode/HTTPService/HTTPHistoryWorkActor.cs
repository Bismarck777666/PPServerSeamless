...
namespace CommNode.HTTPService
{
    class HTTPHistoryWorkActor : ReceiveActor
    {
        ...
        public HTTPHistoryWorkActor(IActorRef dbReaderProxy)
        {
            ...
            ReceiveAsync<HTTPPPHistoryGetItemDetailRequestByRoundID>    (onPPHistoryGetItemDetailRequestByRoundID);

            
            ReceiveAsync<HTTPPPHistoryGetByRoundRequest>    (onPPHistoryByRoundID);
        ...
        private async Task onPPHistoryGetItemDetailRequestByRoundID(HTTPPPHistoryGetItemDetailRequestByRoundID request)
        {
            try
            {
            ...
            {
                _logger.Error("Exception has been occurred in HTTPHistoryWorkActor::onPPHistoryGetItemDetailRequestByRoundID {0}", ex);
        ...
        private async Task onPPHistoryByRoundID(HTTPPPHistoryGetByRoundRequest request)
        {
            try
            {

                PPGameLogHistoryItem gameLogItem = await _dbReaderProxy.Ask<PPGameLogHistoryItem>(request, TimeSpan.FromSeconds(15));
                Sender.Tell(new HttpResponseMessage() { Content = new StringContent(JsonConvert.SerializeObject(gameLogItem)) });
            }
            catch (Exception ex)
            {
                _logger.Error("Exception has been occurred in HTTPHistoryWorkActor::onPPHistoryByRoundID {0}", ex);
                Sender.Tell(new HttpResponseMessage() { Content = new StringContent(""), StatusCode = System.Net.HttpStatusCode.Unauthorized });
            }
        }
