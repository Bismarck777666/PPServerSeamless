...
{
    public class AllwaysWinBetInfo : BaseAmaticSlotBetInfo
    {
        public override int RelativeTotalBet    => 1;
    }

    class AllwaysWinGameLogic : BaseAmaticSlotGame
    {
        #region 게임고유속성값
        ...
        public AllwaysWinGameLogic()
        {
            _gameID     = GAMEID.AllwaysWin;
            GameName    = "AllwaysWin";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strUserID, Currencies currency)
        {
            try
            {
                AllwaysWinBetInfo betInfo   = new AllwaysWinBetInfo();
                ...
                {
                    _logger.Error("{0} betInfo 0 or infinite in AllwaysWinGameLogic::readBetInfoFromMessage", strUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in AllwaysWinGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
