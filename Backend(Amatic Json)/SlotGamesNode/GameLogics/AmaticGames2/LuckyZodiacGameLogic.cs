    ...
    class LuckyZodiacGameLogic : BaseSelFreeAmaticSlotGame
    {
        ...
        protected override void onFreeSpinOptionSelectRequest(string strUserID, int companyID, GITMessage message, UserBonus userBonus, double userBalance)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} option select error in LuckyZodiacGameLogic::onFreeSpinOptionSelectRequest", strUserID);
                }
                else
                {
        ...
        protected override string buildInitString(string strUserID, double balance, Currencies currency)
        {
            ...
            LuckyZodiacInitPacket initPacket = new LuckyZodiacInitPacket(initString, Cols, FreeCols, ReelSetBitNum);
        ...
        public class LuckyZodiacInitPacket : InitPacket
        {
            ...
            public LuckyZodiacInitPacket(string message, int reelCnt, int freeReelCnt, int reelsetBitCnt) : base(message, reelCnt, freeReelCnt, reelsetBitCnt)
            {
                extrasymbol = 0;
                extracolcnt = 0;
                extrawin    = 0;
