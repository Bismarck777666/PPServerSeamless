    ...
    class RouletteRoyalGameLogic : IGameLogicActor
    {
        ...
        protected override async Task onProcMessage(string strUserID, int companyID, GITMessage message, UserBonus userBonus, double userBalance, Currencies currency)
        {
            if (message.MsgCode == (ushort)CSMSG_CODE.CS_AMATIC_DOINIT)
                ...
                await onDoSpin(strUserID, companyID, message, userBalance, currency);
        }
        ...
        protected virtual async Task onDoSpin(string strUserID, int companyID, GITMessage message, double userBalance, Currencies currency)
        {
            try
            {
                ...
                await spinGame(strUserID, companyID, userBalance);
            }
            catch (Exception ex)
            {
        ...
        protected async Task spinGame(string strUserID, int websiteID, double userBalance)
        {
            try
            {
                ...
                sendGameResult(spinResult, totalBet * getPointUnit(betInfo), spinResult.TotalWin, strGameLog);
        ...
        protected async Task<RouletteRoyalResult> generateSpinResult(RouletteRoyalBetInfo betInfo, string strUserID, int websiteID, double userBalance)
        {
            ...
            if (_dicUserResultInfos.ContainsKey(strUserID))
            {
                foreach (int winNum in _dicUserResultInfos[strUserID].WinNumberLogs)
                    winNumberLogs.Add(winNum);
            }

            if (await checkWebsitePayoutRate(websiteID, totalBet * pointUnit, totalWin * pointUnit))
            {
                result = calculateResult(betInfo, strUserID, userBalance + (totalWin - totalBet) * pointUnit, totalWin, rndNum);
            ...
            result = calculateResult(betInfo, strUserID, userBalance + (emptyWin - totalBet) * pointUnit, emptyWin, emptyNum);
        ...
        protected RouletteRoyalResult calculateResult(RouletteRoyalBetInfo betInfo, string strUserID, double userBalance, long relWinMoney, int rnd)
        {
            try
            {
                ...
                spinResult.ResultString = buildResMsgString(betInfo, strUserID, userBalance, rnd, relWinMoney, AmaticMessageType.FreeTrigger);
        ...
        protected void sendGameResult(RouletteRoyalResult spinResult, double betMoney, double winMoney, string strGameLog)
        {
        ...
        protected string buildInitString(string strUserID, double balance, Currencies currency)
        {
            ...
            {
                RouletteRoyalBetInfo betInfo    = _dicUserBetInfos[strUserID];
                RouletteRoyalResult spinResult  = _dicUserResultInfos[strUserID];
        ...
        protected string buildResMsgString(RouletteRoyalBetInfo betInfo, string strUserID, double balance,int winNumber, long winMoney, AmaticMessageType type)
        {
            ...
            if (type == AmaticMessageType.HeartBeat && _dicUserResultInfos.ContainsKey(strUserID))
            {
