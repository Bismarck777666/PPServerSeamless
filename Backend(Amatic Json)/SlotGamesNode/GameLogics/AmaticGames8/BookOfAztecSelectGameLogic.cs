...
namespace SlotGamesNode.GameLogics
{
    class BookOfAztecSelectGameLogic : BaseAmaticSlotGame
    {
        ...
        protected override async Task onDoSpin(string strUserID, int companyID, GITMessage message, UserBonus userBonus, double userBalance, Currencies currency)
        {
            try
            {
                if (message.MsgCode == (ushort)CSMSG_CODE.CS_AMATIC_FSOPTION)
                {
                    ...
                    {
                        _logger.Error("{0} option select error in BookOfAztecSelectGameLogic::onDoSpin", strUserID);
                    }
                    
                    BaseAmaticSlotBetInfo betInfo = _dicUserBetInfos[strUserID];
                    preprocessResponseList(betInfo.RemainReponses, selectOption);
                }
                await base.onDoSpin(strUserID, companyID, message, userBonus, userBalance, currency);
            }
            catch (Exception ex)
            {
        ...
        protected override string buildInitString(string strUserID, double balance, Currencies currency)
        {
            ...
            BookOfAztecSelectInitPacket initPacket = new BookOfAztecSelectInitPacket(initString, Cols, FreeCols, ReelSetBitNum);
        ...
        public class BookOfAztecSelectInitPacket : InitPacket
        {
            ...
            public BookOfAztecSelectInitPacket(string message, int reelCnt, int freeReelCnt, int reelsetBitCnt) : base(message, reelCnt, freeReelCnt, reelsetBitCnt)
            {
