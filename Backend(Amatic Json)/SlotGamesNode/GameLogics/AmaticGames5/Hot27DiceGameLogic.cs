...
{
    public class Hot27DiceBetInfo : BaseAmaticSlotBetInfo
    {
        public override int RelativeTotalBet    => 1;
    }

    class Hot27DiceGameLogic : BaseAmaticSlotGame
    {
        #region 게임고유속성값
        ...
        public Hot27DiceGameLogic()
        {
            _gameID     = GAMEID.Hot27Dice;
            GameName    = "Hot27Dice";
        }
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strUserID, Currencies currency)
        {
            try
            {
                Hot27DiceBetInfo betInfo   = new Hot27DiceBetInfo();
                ...
                {
                    _logger.Error("{0} betInfo 0 or infinite in Hot27DiceGameLogic::readBetInfoFromMessage", strUserID);
                    return;
                }
            ...
            {
                _logger.Error("Exception has been occurred in Hot27DiceGameLogic::readBetInfoFromMessage {0}", ex);
            }
        }
