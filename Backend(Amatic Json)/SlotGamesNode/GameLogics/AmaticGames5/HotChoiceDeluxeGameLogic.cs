    ...
    class HotChoiceDeluxeGameLogic : BaseSelFreeAmaticSlotGame
    {
        ...
        protected override void onFreeSpinOptionSelectRequest(string strUserID, int companyID, GITMessage message, UserBonus userBonus, double userBalance)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} option select error in HotChoiceDeluxeGameLogic::onFreeSpinOptionSelectRequest", strUserID);
                }
                else
                {
        ...
        protected override string buildInitString(string strUserID, double balance, Currencies currency)
        {
            ...
            HotChoiceDeluxeInitPacket initPacket = new HotChoiceDeluxeInitPacket(initString, Cols, FreeCols, ReelSetBitNum);
        ...
        public class HotChoiceDeluxeInitPacket : InitPacket
        {
            ...
            public HotChoiceDeluxeInitPacket(string message, int reelCnt, int freeReelCnt, int reelsetBitCnt) : base(message, reelCnt, freeReelCnt, reelsetBitCnt)
            {
                freeoptionsymbol    = 0;
                curpoint            = curpoint + 2;
            }
