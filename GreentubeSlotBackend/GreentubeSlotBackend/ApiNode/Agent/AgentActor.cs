    ...
    class AgentActor : ReceiveActor
    {
        ...
        private void onGetGameList(GetGameListRequest request)
        {
            try
            {
                ...
                foreach(GameInfo gameInfo in gameInfos)
                {
                    ...
                    apiGame.vendorid = "Greentube";
                    apiGame.gameid   = gameInfo.GameID;
                    apiGame.symbol   = gameInfo.Symbol;
                    apiGame.iconurl1 = string.Format("https://greenfront.jcplay.vip/Content/icons/{0}.png", gameInfo.Symbol);
                    apiGame.iconurl2 = string.Format("https://greenfront.jcplay.vip/Content/icons/{0}.png", gameInfo.Symbol);
        ...
        private async Task onUserAuthRequest(UserAuthRequest request)
        {
            try
            {
                ...
                gameInfo.Symbol = gameInfo.Symbol.Replace("\r", "").Replace("\n", "").Replace("\t", "");
                ...
                string strGameURL   = string.Format("{0}/greentube/openGame?agentid={1}&userid={2}&token={3}&symbol={4}&currency={5}&lang={6}&lobbyurl={7}",
