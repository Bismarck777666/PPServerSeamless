    ...
    class FiveRabbitsMegaGameLogic : BaseSelFreePPSlotGame
    {
        ...
        protected override void readBetInfoFromMessage(GITMessage message, string strUserID)
        {
            try
            {
                ...
                {
                    _logger.Error("{0} betInfo.BetPerLine <= 0 in FiveLionsMegaGameLogic::readBetInfoFromMessage {1}", strUserID, betInfo.BetPerLine);
                    return;
                }

                if (betInfo.MoreBet && betInfo.PurchaseFree)
                {
                    _logger.Error("{0} betInfo.MoreBet and  PurchasedFreeSpin is same time true in FiveLionsMegaGameLogic::readBetInfoFromMessage", strUserID);
                    return;
                }
