    ...
    public class UserActor : ReceiveActor
    {
        ...
        protected async Task onBetWin(BetWinRequest request)
        {
            try
            {
                ...
                {
                    Sender.Tell(new WithdrawResponse(11, "Duplicate transaction"));
                    return;
                }
                if (_balance.LT((double)request.betAmount, _epsilion))
                {
                    Sender.Tell(new WithdrawResponse(6, "Insufficient funds"));
                    return;
                }
                ...
                var response                    = new BetWinResponse(0, "");
        ...
        protected async Task onWithdraw(WithdrawRequest request)
        {
            try
            {
                ...
                var response                    = new WithdrawResponse(0, "");
                response.balance                = (decimal) Math.Round(_balance, 2);
                response.platformTransactionID  = Guid.NewGuid().ToString().Replace("-", "");
                _lastActiveTime                 = DateTime.Now;
                Sender.Tell(response);
        ...
        protected async Task onDeposit(DepositRequest request)
        {
            try
            {
                ...
                if (!redisValue.IsNullOrEmpty)
                {
                    var depositResponse                     = new DepositResponse(11, "Duplicate transaction");
                ...
                var response                    = new DepositResponse(0, "");
                response.balance                = (decimal)Math.Round(_balance, 2);
                response.platformTransactionID  = Guid.NewGuid().ToString().Replace("-", "");
                _lastActiveTime                 = DateTime.Now;
                Sender.Tell(response);
        ...
        protected async Task onRollback(RollbackRequest request)
        {
            try
            {
                ...
                if (redisValue.IsNullOrEmpty)
                {
                    Sender.Tell(new RollbackResponse(8, "Could not find reference transaction id"));
                    return;
                }
                if (redisValue == "Rollbacked")
                {
                    Sender.Tell(new RollbackResponse(9, "Transaction is already rolled back"));
                    return;
                }
                ...
                Sender.Tell(new RollbackResponse(0, ""));
            }
            catch
            {
